...
class MY_Model extends CI_Model
{
	...
	function send_email($subject, $email_content, $user_id, $check_replace = true){
		...
		if(get_role("m-seller")){
		    $user_id=trim(session('uid'));
		}
		 $user_info = $this->get("first_name, last_name, email, timezone, reset_key, activation_key", USERS, "id = '{$user_id}'");
		 
		$emailtype=$this->db->select("lang")->from(USERS)->where("role",'admin')->get()->row();	
		
		$sid=trim(session('uid'));
		if (empty($user_info)) {
			return "Account does not exists!";
		}
		/*----------  Get Mail Template  ----------*/
		//$mail_template = file_get_contents(APPPATH.'/libraries/PHPMailer/template.php');
		if(($emailtype->lang)=='en'){
			$mail_template = file_get_contents(APPPATH.'/libraries/PHPMailer/template.php');
		}else{
			$mail_template = file_get_contents(APPPATH.'/libraries/PHPMailer/template_ar.php');
		}
		
		/*----------  replace variable in email content, subject  ----------*/
		if(get_role("m-seller")){
			$email_from = get_option('email_from_'.$sid, '')?get_option('email_from', ''):"do-not-reply@smm.com";
			$email_name = get_option('email_name_'.$sid, '')?get_option('email_name', ''):get_option('website_title', '');
