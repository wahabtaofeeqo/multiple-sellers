...
class api_provider extends MX_Controller {
	...
	public function ajax_api_provider_services($ids = ""){
		if (!empty($ids)) {
			...
			if (!empty($api)) {
				...
				$categories  = $this->model->fetch("*", $this->tb_categories, "status = 1", 'sort','ASC','group_by','name');
				$data = array(
					"api_id"	 => $api->id,
					"api_ids"	 => $api->ids,
					"module"     => get_class($this),
	...
	public function cron($type = "",$order_id='',$api_id=''){//api id call from api.php for ticket
		switch ($type) {
			case 'order':
