...
class MY_Model extends CI_Model
{
	...
	function send_email($subject, $email_content, $user_id, $check_replace = true,$login_uid=false){
		...
		try {
			...
			if($email_protocol_type == "smtp" && $smtp_server != "" && $smtp_port != "" && $smtp_username != "" && $smtp_password != ""){
				...
				$mail->isSendmail();
                    ...
                    if($login_uid !=false){
                        $login_user_info = $this->get("first_name, last_name, email, timezone, reset_key, activation_key", USERS, "id = '".$login_uid."'");
                        $email_from = $login_user_info->email;
                        $email_name = $login_user_info->first_name;
                    }
		    //Recipients
