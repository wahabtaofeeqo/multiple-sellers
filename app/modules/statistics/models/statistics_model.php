...
class statistics_model extends MY_Model {
	...
	public function get_data_logs() {
		...
		if (get_role("m-seller")) {
		   $total_order_spent_receive 	= $this->get_sum_order($this->tb_orders);
		}
		...
		if (get_role("m-seller")) {
			 ...
			 $val=0;
			//if($total_spent_receive!=0){
				$val = $total_order_spent_receive;
				if($val < 0) {
					$val=0;
				}
			//}
			...
			$data = (object)array(
				...
				"total_spent_receive"   => $val,
				"user_balance"          => $total_spent_receive,
			);
			...
			$data = (object)array(
				...
				"total_users"     		=> $total_users,
				"total_transactions"    => $total_transactions,
				"total_spent_receive"   => $total_spent_receive + $total_order_spent_receive,
		...
		if (!get_role("admin")) {
			...
			foreach ($result as $key => $row) {
					if($row->type=='paypal'){
						...
						$tot+=round((($row->amount+0.3)/(100-get_option("paypal_chagre_fee")))*100);
						
					}
					if($row->type=='stripe'){
						
						$tot+=round((($row->amount+0.3)/(100-get_option("stripe_chagre_fee")))*100);
						
					}
					if($row->type=='two_checkout'){
						
						$tot+=round((($row->amount+0.3)/(100-get_option("two_checkout_chagre_fee")))*100);
	...
	private function get_sum_order($table){
		if (get_role("m-seller")) {
			$this->db->where("uid", session('uid'));
