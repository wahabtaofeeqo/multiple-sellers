<?php
defined('BASEPATH') OR exit('No direct script access allowed');

class category extends MX_Controller {
    public $tb_users;
    public $tb_categories;
    public $tb_services;
    public $tb_sub_categories;
    public $tb_second_sub_categories;
    public $columns;
    public $module_name;
    public $module_icon;

    public function __construct(){
        parent::__construct();
        $this->load->model(get_class($this).'_model', 'model');

        //Config Module
        $this->tb_categories = CATEGORIES;
        $this->tb_sub_categories = SUB_CATEGORIES;
        $this->tb_second_sub_categories = SECOND_SUB_CATEGORIES;
        $this->tb_services   = SERVICES;
		$this->tb_users   = general_users;
        $this->module_name   = 'Category';
        $this->module_icon   = "fa ft-users";
        $this->columns = array(
            "name"             => lang("Name"),
            "desc"             => lang("Description"),
            "sort"             => lang("Sorting"),
            "status"           => lang("Status"),
        );
    }

    public function index(){
        $page        = (int)get("p");
        $page        = ($page > 0) ? ($page - 1) : 0;
        $limit_per_page = get_option("default_limit_per_page", 10);
        $query = array();
        $query_string = "";
        if(!empty($query)){
            $query_string = "?".http_build_query($query);
        }
        $config = array(
            'base_url'           => cn(get_class($this).$query_string),
            'total_rows'         => $this->model->get_category_lists(true),
            'per_page'           => $limit_per_page,
            'use_page_numbers'   => true,
            'prev_link'          => '<i class="fe fe-chevron-left"></i>',
            'first_link'         => '<i class="fe fe-chevrons-left"></i>',
            'next_link'          => '<i class="fe fe-chevron-right"></i>',
            'last_link'          => '<i class="fe fe-chevrons-right"></i>',
        );
        $this->pagination->initialize($config);
        $links = $this->pagination->create_links();

        $categories = $this->model->get_category_lists(false, "all", $limit_per_page, $page * $limit_per_page);
        $sub_categories = $this->model->get_sub_category_lists();
        $second_sub_categories = $this->model->get_second_sub_category_lists();
		
        $data = array(
            "module"     => get_class($this),
            "columns"    => $this->columns,
            "categories" => $categories,
            "from"       => $page * $limit_per_page,
            "links"      => $links,
            "sub_categories"=>$sub_categories,
            "second_sub_categories"=>$second_sub_categories
        );

        $this->template->build('index', $data);
    }
    public function update($ids = ""){

        $category = $this->model->get("*", $this->tb_categories, "ids = '{$ids}'");
       
		$this->db->select('*');
		$this->db->where('role','m-seller');
		$res_img = $this->db->get($this->tb_users);
		$mSellers = $res_img->result_array();
		
        $data = array(
            "module"   => get_class($this),
			"mSellers"   => $mSellers,
            "category" => $category,
        );
        $this->load->view('update', $data);
    }
    public function update_sub_category($id = ""){
        $sub_category = $this->model->get("*", $this->tb_sub_categories, "id = '{$id}'");
        $categories = $this->model->get_category_lists(false, "all");
		$this->db->select('*');
		$this->db->where('role','m-seller');
		$res_img = $this->db->get($this->tb_users);
		$mSellers = $res_img->result_array();
		
        $data = array(
            "module"   => get_class($this),
            "sub_category" => $sub_category,
			"mSellers"   => $mSellers,
            "categories"=>$categories
        );
        $this->load->view('sub_category_update', $data);
    }
    public function update_second_sub_category($id = ""){
        $second_sub_category = $this->model->get("*", $this->tb_second_sub_categories, "id = '{$id}'");
        //$sub_categories = $this->model->get_sub_category_lists(false, "all");
        $categories = $this->model->get_category_lists(false, "all");
		//added bu mithu0705
		$check_item = $this->model->get("cat_id", $this->tb_sub_categories, "id = '{$second_sub_category->sub_cat_id}'");
		$sub_categories = $this->model->get_sub_category_by($check_item->cat_id);
		
		$this->db->select('*');
		$this->db->where('role','m-seller');
		$res_img = $this->db->get($this->tb_users);
		$mSellers = $res_img->result_array();
        $data = array(
            "module"   => get_class($this),
            "sub_categories" => $sub_categories,
            "categories"=>$categories,
			"mSellers"   => $mSellers,
            'second_sub_category'=>$second_sub_category
        );
        $this->load->view('sub_category_2_update', $data);
    }
    public function ajax_update_second_sub_category($id = ""){
        $name 		= post("name");
        $image	    = post("image");
        $sort 		= (int)post("sort");
        $status 	= (int)post("status");
        $sub_category_id=(int)post("sub_category_id");
        $desc 		= post("desc");
		$seller_permission=post('seller_permission');
		$seller_permission_str = implode (",", $seller_permission);
		
        if($name == ""){
            ms(array(
                "status"  => "error",
                "message" => lang("name_is_required")
            ));
        }

        // if($image == ""){
        // 	ms(array(
        // 		"status"  => "error",
        // 		"message" => "Imgage logo is required"
        // 	));
        // }

        if($sort == "" || $sort <= 0){
            ms(array(
                "status"  => "error",
                "message" => lang("sort_number_must_to_be_greater_than_zero")
            ));
        }

        //
        $data = array(
            "uid"             => session('uid'),
            "name"            => $name,
            "desc"            => $desc,
            "image"           => $image,
            "status"          => $status,
            "seller_permission" => $seller_permission_str,
			"sort"            => $sort,
            "sub_cat_id"          =>$sub_category_id
        );

        $check_item = $this->model->get("*", $this->tb_second_sub_categories, "id = '{$id}'");

        if(empty($check_item)){

            $data["ids"]     = ids();
            $data["changed"] = NOW;
            $data["created"] = NOW;

            $this->db->insert($this->tb_second_sub_categories, $data);
        }else{

            $data["changed"] = NOW;
            $this->db->update($this->tb_second_sub_categories, $data, array("id" => $check_item->id));
            if ($status != 1 ) {
                $this->db->update($this->tb_services, ["status" => 0], ["cate_id" => $check_item->id]);
            }
        }

        ms(array(
            "status"  => "success",
            "message" => lang("Update_successfully")
        ));
    }
    public function ajax_update_sub_category($id = ""){
        $name 		= post("name");
        $image	    = post("image");
        $sort 		= (int)post("sort");
        $status 	= (int)post("status");
        $category_id=(int)post("category_id");
        $desc 		= post("desc");
		$seller_permission=post('seller_permission');
		$seller_permission_str = implode (",", $seller_permission);
		
        if($name == ""){
            ms(array(
                "status"  => "error",
                "message" => lang("name_is_required")
            ));
        }

        // if($image == ""){
        // 	ms(array(
        // 		"status"  => "error",
        // 		"message" => "Imgage logo is required"
        // 	));
        // }

        if($sort == "" || $sort <= 0){
            ms(array(
                "status"  => "error",
                "message" => lang("sort_number_must_to_be_greater_than_zero")
            ));
        }

        //
        $data = array(
            "uid"             => session('uid'),
            "name"            => $name,
            "desc"            => $desc,
            "image"           => $image,
            "status"          => $status,
			"seller_permission" => $seller_permission_str,
            "sort"            => $sort,
            "cat_id"          =>$category_id
        );

        $check_item = $this->model->get("*", $this->tb_sub_categories, "id = '{$id}'");

        if(empty($check_item)){

            $data["ids"]     = ids();
            $data["changed"] = NOW;
            $data["created"] = NOW;

            $this->db->insert($this->tb_sub_categories, $data);
        }else{

            $data["changed"] = NOW;
            $this->db->update($this->tb_sub_categories, $data, array("id" => $check_item->id));
            if ($status != 1 ) {
                $this->db->update($this->tb_services, ["status" => 0], ["cate_id" => $check_item->id]);
            }
        }

        ms(array(
            "status"  => "success",
            "message" => lang("Update_successfully")
        ));
    }
    public function add_sub_category(){
		$this->db->select('*');
		$this->db->where('role','m-seller');
		$res_img = $this->db->get($this->tb_users);
		$mSellers = $res_img->result_array();
		
        $categories = $this->model->get_category_lists(false, "all");

        $data = array(
            "module"   => get_class($this),
			"mSellers"   => $mSellers,
            "categories" => $categories,
        );
        $this->load->view('add_sub_category', $data);
    }
    public function add_second_sub_category(){
        $categories = $this->model->get_category_lists(false, "all");
        $sub_categories = $this->model->get_sub_category_lists(false, "all");
		$this->db->select('*');
		$this->db->where('role','m-seller');
		$res_img = $this->db->get($this->tb_users);
		$mSellers = $res_img->result_array();
		
        $data = array(
            "module"   => get_class($this),
            "sub_categories" => $sub_categories,
			"mSellers"   => $mSellers,
            "categories"=>$categories
        );
        $this->load->view('add_second_sub_category', $data);
    }
    public function ajax_add_sub_category(){
        $lang_current = get_lang_code_defaut();
        $str=rand();
        $result = md5($str);
        $name 		= post("name");
        $image	    = post("image");
        $sort 		= (int)post("sort");
        $status 	= (int)post("status");
        $desc 		= post("desc");
        $cat_id 		= post("cateegory_id");
        $sub_cat_lang   = $lang_current->code;
		$seller_permission=post('seller_permission');
		$seller_permission_str = implode (",", $seller_permission);
		
        $ids=$a = $result;

        if($name == ""){
            ms(array(
                "status"  => "error",
                "message" => lang("name_is_required")
            ));
        }


        if($sort == "" || $sort <= 0){
            ms(array(
                "status"  => "error",
                "message" => $sort
            ));
        }

        //
        $data = array(
            "ids"=>$ids,
            "uid"             => session('uid'),
            "name"            => $name,
            "desc"            => $desc,
            "image"           => $image,
            "status"          => $status,
            "sort"            => $sort,
            "cat_id"          => $cat_id,
            "seller_permission" => $seller_permission_str,
            "lang"            => $sub_cat_lang
        );

        $check_item = $this->model->get("id, ids", "sub_categories", "name = '{$name}'");

        if(!empty($check_item)){

            ms(array(
                "status"  => "error",
                "message" => lang("name_already_exists")
            ));


        }
		
        $this->db->insert("sub_categories", $data);

        ms(array(
            "status"  => "success",
            "message" => lang("Update_successfully")
        ));

    }
    public function ajax_add_second_sub_category(){
        $lang_current = get_lang_code_defaut();
        $str=rand();
        $result = md5($str);
        $name 		= post("name");
        $image	    = post("image");
        $sort 		= (int)post("sort");
        $status 	= (int)post("status");
        $desc 		= post("desc");
        $cat_id 		= post("sub_cateegory_id");
        $sec_sub_cat_lang   = $lang_current->code;
		$seller_permission=post('seller_permission');
		$seller_permission_str = implode (",", $seller_permission);
        
        $ids=$a = $result;
        if($name == ""){
            ms(array(
                "status"  => "error",
                "message" => lang("name_is_required")
            ));
        }



        if($sort == "" || $sort <= 0){
            ms(array(
                "status"  => "error",
                "message" => $sort
            ));
        }

        //
        $data = array(
            "ids"=>$ids,
            "uid"             => session('uid'),
            "name"            => $name,
            "desc"            => $desc,
            "image"           => $image,
            "status"          => $status,
            "sort"            => $sort,
            "sub_cat_id"            => $cat_id,
			"seller_permission" => $seller_permission_str,
            "lang"              =>$sec_sub_cat_lang
        );

        $check_item = $this->model->get("id, ids", "second_sub_categories", "name = '{$name}'");

        if(!empty($check_item)){

            ms(array(
                "status"  => "error",
                "message" => lang("name_already_exists")
            ));


        }

        $this->db->insert("second_sub_categories", $data);

        ms(array(
            "status"  => "success",
            "message" => lang("Update_successfully")
        ));

    }
    public function ajax_update($ids = "") {
        
        $lang_current = get_lang_code_defaut();
        
        $name 		= post("name");
        $image	    = post("image");
        $sort 		= (int)post("sort");
        $status 	= (int)post("status");
        $desc 		= post("desc");
        $cat_lang   = $lang_current->code;

		$seller_permission = post('seller_permission');
		$seller_permission_str = implode (",", $seller_permission);

        if($name == ""){
            ms(array(
                "status"  => "error",
                "message" => lang("name_is_required")
            ));
        }

        // if($image == ""){
        // 	ms(array(
        // 		"status"  => "error",
        // 		"message" => "Imgage logo is required"
        // 	));
        // }

        if($sort == "" || $sort <= 0){
            ms(array(
                "status"  => "error",
                "message" => lang("sort_number_must_to_be_greater_than_zero")
            ));
        }
    
        //
        $data = array(
            "uid"             => session('uid'),
            "name"            => $name,
            "desc"            => $desc,
            "image"           => $image,
            "status"          => $status,
            "sort"            => $sort,
            "seller_permission" => $seller_permission_str,
            "lang"            => $cat_lang
        );

        $check_item = $this->model->get("id, ids", $this->tb_categories, "ids = '{$ids}'");

        if(empty($check_item)){

            $data["ids"]     = ids();
            $data["changed"] = NOW;
            $data["created"] = NOW;

             
            $this->db->insert($this->tb_categories, $data);
        }else{
            $data["changed"] = NOW;
            $this->db->update($this->tb_categories, $data, array("ids" => $check_item->ids));
            if ($status != 1 ) {
                $this->db->update($this->tb_services, ["status" => 0], ["cate_id" => $check_item->id]);
            }
        }

        ms(array(
            "status"  => "success",
            "message" => lang("Update_successfully")
        ));
    }
    public function ajax_search(){
        $k = post("k");
        $categories = $this->model->get_category_lists_by_search($k);
        $data = array(
            "module"     => get_class($this),
            "columns"    => $this->columns,
            "categories" => $categories,
        );
        $this->load->view("ajax_search", $data);
    }
    public function ajax_delete_second_sub_category_item($id = ""){
        $sub_category = $this->model->get("*", $this->tb_second_sub_categories, "id = '{$id}'");
        $ids=$sub_category->ids;
        $this->db->delete('second_sub_categories', array('id' => $id));
        ms(array(
            "status"  => "success",
            "message" => lang("Deleted_successfully"),
            'ids'=>$ids
        ));
    }
    public function ajax_delete_sub_category_item($id = ""){
        $sub_category = $this->model->get("*", $this->tb_sub_categories, "id = '{$id}'");
        $ids=$sub_category->ids;
        $this->db->delete('sub_categories', array('id' => $id));
        $this->db->delete('second_sub_categories', array('sub_cat_id' => $id));
        ms(array(
            "status"  => "success",
            "message" => lang("Deleted_successfully"),
            'ids'=>$ids
        ));
    }

    public function ajax_delete_item($ids = "") {
        
        $category = $this->model->getCategoryWithIDs($ids);
        $response = $this->model->getAdminKey();
        $post = array(
            'name' => $category->name);

        $data = array(
            'key' => $response->key,
            'action' => 'delete-category',
            'post_data' => json_encode($post));

       $this->connectAPI(main_admin_api_endpoint(), $data);

        $this->model->deleteServicesByCategoryID($category->id);
        $this->model->delete($this->tb_categories, $ids, false);

        ms(array(
            "status"  => "success",
            "message" => lang("Deleted_successfully"),

        ));
    }

    public function ajax_actions_option(){
        $type = post("type");
        $idss = post("ids");
        if ($type == '') {
            ms(array(
                "status"  => "error",
                "message" => lang('There_was_an_error_processing_your_request_Please_try_again_later')
            ));
        }

        if (in_array($type, ['delete', 'deactive', 'active']) && empty($idss)) {
            ms(array(
                "status"  => "error",
                "message" => lang("please_choose_at_least_one_item")
            ));
        }
        switch ($type) {
            case 'delete':
                foreach ($idss as $key => $ids) {
                    /*----------  delete all related services  ----------*/
                    $item = $this->model->get("id, ids", $this->tb_categories, ['ids' => $ids]);
                    if (!empty($item)) {
                        $this->db->delete($this->tb_services, ["cate_id" => $item->id]);
                    }
                    $this->db->delete($this->tb_categories, ['ids' => $ids]);
                }
                ms(array(
                    "status"  => "success",
                    "message" => lang("Deleted_successfully")
                ));
                break;
            case 'deactive':
                foreach ($idss as $key => $ids) {
                    /*----------  deactive all related services  ----------*/
                    $item = $this->model->get("id, ids", $this->tb_categories, ['ids' => $ids]);
                    if (!empty($item)) {
                        $this->db->update($this->tb_services, ['status' => 0], ["cate_id" => $item->id]);
                    }

                    $this->db->update($this->tb_categories, ['status' => 0], ['ids' => $ids]);
                }
                ms(array(
                    "status"  => "success",
                    "message" => lang("Updated_successfully")
                ));
                break;

            case 'active':
                foreach ($idss as $key => $ids) {
                    /*----------  active all related services  ----------*/
                    $item = $this->model->get("id, ids", $this->tb_categories, ['ids' => $ids]);
                    if (!empty($item)) {
                        $this->db->update($this->tb_services, ['status' => 1], ["cate_id" => $item->id]);
                    }

                    $this->db->update($this->tb_categories, ['status' => 1], ['ids' => $ids]);
                }
                ms(array(
                    "status"  => "success",
                    "message" => lang("Updated_successfully")
                ));
                break;


            case 'all_deactive':
                $categories = $this->model->fetch("id, ids", $this->tb_categories, ['status' => 0]);
                if (empty($categories)) {
                    ms(array(
                        "status"  => "error",
                        "message" => lang("failed_to_delete_there_are_no_deactivate_category_now")
                    ));
                }

                /*----------  delete all related services  ----------*/
                foreach ($categories as $key => $row) {
                    $item = $this->model->get("id, ids", $this->tb_categories, ['ids' => $row->ids]);
                    if (!empty($item)) {
                        $this->db->delete($this->tb_services, ["cate_id" => $item->id, 'status' => 0]);
                    }
                }

                $this->db->delete($this->tb_categories, ['status' => 0]);
                ms(array(
                    "status"  => "success",
                    "message" => lang("Deleted_successfully")
                ));

                break;

            default:
                ms(array(
                    "status"  => "error",
                    "message" => lang('There_was_an_error_processing_your_request_Please_try_again_later')
                ));
                break;
        }

    }
	
	//added bu mithu0705
	public function GetSubCat()
	{
	 	$main_cat=$this->input->post('cate');		
		$sub_categories=$this->model->GetSubCatm($main_cat);
			
		foreach ($sub_categories->result_array() as $key => $value) {
			echo "<option value='" .  $value['id'] . "'>" . $value['name'] . "</option>";
		}
	}

    public function connectAPI($url, $data) {

        $curl = curl_init();
        curl_setopt_array($curl, [
                CURLOPT_RETURNTRANSFER => 1,
                CURLOPT_URL => $url,
                CURLOPT_POST => 1,
                CURLOPT_POSTFIELDS => $data
            ]
        );

        $response = curl_exec($curl);
        $err = curl_error($curl);
        // print_r($response);
        // print_r($err);

        curl_close($curl);
    }
}