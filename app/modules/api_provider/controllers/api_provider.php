...
class api_provider extends MX_Controller {
    ...
    private function sync_services_by_api($data_item) {
        ...
        $disabled_services = array_udiff($services, $api_services, function ($obj_a, $obj_b) {
            return $obj_a->service - $obj_b->service;
        }
        );

        $new_services = array_udiff($api_services, $services, function ($obj_a, $obj_b) {
            return $obj_a->service - $obj_b->service;
        }
        );

        $exists_services = array_udiff($api_services, $new_services, function ($obj_a, $obj_b) {
            return $obj_a->service - $obj_b->service;
        }
        );
                ...
                foreach ($data_services as $key => $row) {
                    ...
                    if (($chekopt != 3) && ($chekopt != 0)) {
                        ...
                        if ($i < $bulk_limit || $bulk_limit == "all") {
                            ...
                            $apiService = trim($row->service);
                            $apiid = trim($api->id);

                            $check_services = $this->model->get("id, ids, api_provider_id, api_service_id", $this->tb_services, "api_service_id='" . $apiService . "' AND api_provider_id='" . $apiid . "' AND uid = '" . session('uid') . "'");
                            ...
                            if (empty($check_services)) {
                                ...
                                if ($check_st == 1) {
                                    foreach ($s_optArray as $keys => $rows) {

                                        if (trim($rows) === trim($apiService)) {
                                            $this->db->insert($this->tb_services, $data_service);
                                            $this->addmedia($data_media, $row->media_url);
                                        }
                                    }
                                } else {
                                    ...
                                    foreach ($s_optArray as $keys => $rows) {
                                        ...
                                        if (trim($rows) === trim($apiService)) {
                                            $check_services_0 = $this->model->get("id, ids, api_provider_id, api_service_id", $this->tb_services, "api_service_id='" . $apiService . "' AND uid = '" . session('uid') . "'");
                                            if (empty($check_services_0)) {
                                                $this->db->insert($this->tb_services, $data_service);
                                                $this->addmedia($data_media, $row->media_url);
                                            } else {

                                                $check_services_1 = $this->model->get("id, ids, api_provider_id, api_service_id", $this->tb_services, "api_service_id='" . $apiService . "' AND api_provider_id=' ' AND uid = '" . session('uid') . "'");
                        ...
                        if ($i < $bulk_limit || $bulk_limit == "all") {
                            ...
                            if (empty($check_services)) {
                                ...
                                $this->db->update($this->tb_services, $data_service, ["api_service_id" => $api->id, "api_provider_id" => $api->id, "ids" => $check_services->ids]);
    ...
    public function addmedia($datamedia, $id) {


        foreach ($datamedia as $keys => $rows) {

            if (trim($rows->media_id) == trim($id)) {
                $check_media = $this->model->get("id, type", $this->tb_services_media, "name='" . $rows->name . "' AND media_id='" . $id . "' AND uid = '" . session('uid') . "'");
    ...
    public function cron($type = "", $order_id = '', $api_id = '', $forward_call = "true") {//api id call from api.php for ticket
        switch ($type) {
            ...
            case 'single_status_update':
                ...
                /* ----------  Get all order through API  ---------- */
                $orders = $this->model->getOrders($api_order_id);
                $default_price_percentage_increase = get_option("default_price_percentage_increase", 30);
                        ...
                        if (!empty($api)) {
                            ...
                            if (isset($response->status) && $response->status != "") {
                                ...
                                if (!empty($data)) {
                                    ...
                                    $data["comment_link"] = $response->comment_link;
                                    $this->db->update($this->tb_orders, $data, ["id" => $row->id]);
                                    ...
                                    foreach ($client_arr as $client) {
                                        $url_host = $client->api_client;
                                        if ($url_host != '') {
                                            $url = "http://$url_host/api_provider/cron/single_status_update/$row->id";
                                            $this->call_cron($url);
                                        }
                                        break; //only one item
                                    }
            ...
            case 'ticket_update':
                ...
                if (isset($ticket_data[0]["api_provider"]) && $forward_call == "false") {
                    ...
                    //$user = $this->model->get_general_user(session('uid'));
                    $ticket_data[0]["ticket_back"] = true;
                    $ticket_data[0]["api_id"] = $api_id;
                    $ticket_data[0]["ticket_message"] = $ticket_messages;
//                    $ticket_data[0]["sender_first_name"] = '$user->first_name';
//                    $ticket_data[0]["sender_last_name"] = '$user->last_name';
//                    $ticket_data[0]["sender_email"] = '$user->email';
                    $data_post = array(
...
//                                echo var_dump($response);
            ...
            case 'ticket':
                ...
                $ticket_data = $this->model->getTicketWithID($order_id); //order id means ticket id
                $ticket_messages = $this->model->get_messages($order_id); //order id means ticket id

                $subject = $ticket_data->subject;
                $parse = parse_url(base_url());
                
                //Admin Info is used to send the ticket
                $admin = $this->model->get_admin();
                $order = $this->model->getOrder($ticket_data->order_id);
                ...
                if (strpos($subject, "Order") !== false) {
                    ...
                    //Send to main Admin website
                    $url = 'http://localhost/admin/api/v1';
                    $response = $this->connect_api($url, $data_post);
                }
                    ...
                    if (!empty($orders)) {
                        foreach ($orders as $key => $row) {
                            ...
                            if ($row->id == $relevant_id) {
                                $api = $this->model->get("url, key", $this->tb_api_providers, ["id" => $row->api_provider_id]);
//                                $user = $this->model->get_general_user($row->uid);
...
//                                $ticket_data[0]["sender_email"] = $user->email;
                                ...
                                $data_post = array(
                                    'key' => ($api != null) ? $api->key : '',
                                    'action' => 'ticket',
                                    'api_client' => $api_client_url,
                                    'post_data' => json_encode($ticket_data[0])
                                );

                                $url = ($api != null) ? $api->url : '';
                                $response = $this->connect_api($url, $data_post);
                                echo var_dump($response);
                            }
                        }
...
//                    $user = $this->model->get_general_user($ticket_data[0]["uid"]);
                    $ticket_data[0]["api_provider"] = $api_client_url;
                    $ticket_data[0]["api_key"] = $api_client_key;
