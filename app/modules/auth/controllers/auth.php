...
class auth extends MX_Controller {
	...
	public function about(){
		...
		$this->template->build('../../../themes/'.get_theme().'/views/test', $data);
	...
	public function ajax_sign_in(){
		...
		$user = $this->model->get("id, status, ids, email, password, role, first_name, last_name, timezone", $this->tb_users, "email = '{$email}' AND password = '{$password}'");
		if(!empty($user)){
			if($user->status != 1){
				ms(array(
					"status"  => "error",
