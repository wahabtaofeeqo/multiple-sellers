            ...
            <div class="tab-content">
              <form id="paymentFrm" method="post" action="<?=cn($module."/two_checkout/create_payment")?>">
				...
				<?php 
				   ...
				   if (isset($sellerid)) : ?>
                  <label class="form-label"><?=sprintf(lang("amount_usd"), get_option("currency_code_".$sellerid,'USD'))?></label>
                  <?php else: ?>
...
<?php
  if (get_option('payment_environment_'.session("uid"),"") == "sandbox") {
...
var tokenRequest = function() {
  ...
  var args = {
    sellerId: "<?=get_option('2checkout_seller_id_'.session('uid'),"")?>",
    publishableKey: "<?=get_option('2checkout_publishable_key_'.session('uid'),"")?>",
    ccNo: $("#card_num").val(),
    cvv: $("#cvv").val(),
...
$(function() {
  ...
  TCO.loadPubKey("<?=$payment_environment_session('uid')?>");
