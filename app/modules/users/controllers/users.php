...
class users extends MX_Controller {
	...
	public function index(){
		...
		$data = array(
			...
			"links"        => $links,
                ...
                $menu_list    = $this->model->get_menu_list();
	...
	public function update($ids = ""){
		...
		$files    = $this->model->get_file_list($user->id);
	...
	public function ajax_update($ids = ""){
		...
		$desc               = post('desc');
                $menu_permission=post('user_permission');
		if($first_name == '' || $last_name == ''){
			ms(array(
                ...
                if(empty($menu_permission)){
                    ms(array(
                            'status'  => 'error',
                            'message' => lang("please_select_menu_permission"),
                    ));
                }
		...
		$data = array(
			...
			"timezone"                => $timezone,
			"desc"        	          => $desc,
			"menu_permissions"        => $menu_permission_str,
			"changed"                 => NOW,
		);
