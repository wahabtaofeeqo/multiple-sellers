		...
		if (!empty($ids)) {
			...
			if (!empty($api)) {
		        ...
		        if($type=='curr'){
						...
						foreach ($data_services as $key => $row) {
						    $servname=trim($row->service);
							
							$check_services = $this->model->get("id, ids,name,api_provider_id, api_service_id", $this->tb_services, "api_service_id='".$servname."' AND add_type ='api' AND uid = '".session('uid')."'");
						...
						foreach ($data_services as $key => $row) {
						    $servname=trim($row->service);
							$check_services = $this->model->get("id, ids,name,api_provider_id, api_service_id", $this->tb_services, "api_service_id='{$servname}' AND uid = '".session('uid')."'");
				...
				if (!empty($orders)) {
					...
					echo "There is no order at the present.<br>";
                        ...
                        case 'ticket':
							...
							$id_s=explode("-",$ticket_data[0]["subject"]);
							
							$this->db->select('api_service_id');
							$this->db->from('services');
							$this->db->where("id", trim($id_s[1]));
							$query = $this->db->get();
							$ald=$query->row();
							$aserviceid=$ald->api_service_id;
                            ...
                            if(isset($ticket_data[0]["api_provider"]) && $forward_call=="false"){
								...
								$ticket_data[0]["service_id"]=$aserviceid;
                                $data_post = array(
