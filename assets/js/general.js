...
function General(){
    ...
    this.order = function (){
        ...
        $(document).on("input", ".ajaxQuantity" , function(){
            ...
            _total_charge = (_total_quantity != "" && _service_price != "") ? (_total_quantity * _service_price)/1 : 0;
            _total_charge = _total_charge.toFixed(2);
            _currency_symbol = $("#new_order input[name=currency_symbol]").val();
            $("#new_order input[name=total_charge]").val(_total_charge);
            $("#new_order .total_charge span").html(_currency_symbol + parseFloat(_total_charge).toFixed(2));        })
    ...
    this.calculateOrderCharge = function(){
        ...
        $(document).on("keyup", ".ajax_custom_comments" , function(){
            ...
            _total_charge = (_quantity != "" && _service_price != "") ? (_quantity * _service_price)/1 : 0;
            _total_charge = _total_charge.toFixed(2);
            _currency_symbol = $("#new_order input[name=currency_symbol]").val();
            $("#new_order input[name=total_charge]").val(_total_charge);
            $("#new_order .total_charge span").html(_currency_symbol + _total_charge);
    ...
    this.generalOption = function(){
        ...
        $(document).on("change", ".ajaxChangeTicketSubject", function(){
            ...
            switch(_type) {
              ...
              case "subject_order":
                ...
                $("#add_new_ticket .subject-service").addClass("d-none");
                $("#add_new_ticket .subject-payment-other").addClass("d-none");
                break;  
              ...
              case "subject_payment":
                ...
                $("#add_new_ticket .subject-service").addClass("d-none");
                $("#add_new_ticket .subject-payment-other").removeClass("d-none");
                break;
              case "subject_service":
                $("#add_new_ticket .subject-order").addClass("d-none");
                $("#add_new_ticket .subject-payment").addClass("d-none");
                $("#add_new_ticket .subject-service").removeClass("d-none");
                $("#add_new_ticket .subject-payment-other").addClass("d-none");
                break;
        ...
        $(document).on("change", ".ajaxChangeService" , function(event){
            event.preventDefault();
            _that         = $(this);
            _id           = _that.val();
        ...
        // callback actionForm custom
        $(document).on("submit", ".actionFormCustom", function(){
            pageOverlay.show();
            event.preventDefault();
            _that       = $(this);
            _action     = _that.attr("action");
...
//            $.post(_action, data, function(_result){
//
//                setTimeout(function(){
//                    pageOverlay.hide();
//                },1500)
...
//            })
            return false;
                ...
                // callback actionForm custom
        $(document).on("submit", ".actionFormCustomMedia", function(){
            pageOverlay.show();
            event.preventDefault();
            _that       = $(this);
            _action     = _that.attr("action");
